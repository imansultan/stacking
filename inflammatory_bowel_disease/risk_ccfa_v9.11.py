...
def main():
    ...
    dataset_df=pd.read_excel('input_data/sample_meta_data_risk_CCFA.xlsx',
                              index_col='ExampleID')
    dataset_df['tmp'] = dataset_df.index.str[-2:]
    dataset_df=dataset_df.query("tmp != '.b'")
    dataset_df=dataset_df.query("collection == 'RISK'")
    dataset_df=dataset_df.query("antibiotics == 0")
    dataset_df=dataset_df.query("steroids == False")
    dataset_df.drop(columns=['tmp'],
                    inplace=True)
    
    otu_df = pd.read_csv('input_data/otu_RISK_CCFA.txt',
                         index_col='ExampleID',
                         sep="\t")
    ...
    tax_df = pd.read_csv('input_data/tax_RISK_CCFA.txt',
                         index_col='FeatureID',
                         sep="\t")
    ...
    dataset_df=dataset_df.query("diagnosis != 'no'")
    dataset_df=dataset_df.query("diagnosis != 'IC'")
    
    dataset_df.drop(columns=["external_id",
                             "run_prefix",
                             "body_site",
                             "diseasesubtype",
                             "gastrointest_disord",
    ...
    age_df=dataset_df["age_years"].to_frame()
    
    
    
    diagnosis_df=dataset_df["diagnosis"].to_frame()
    
    dataset_clinical_cat_df=dataset_df.drop(columns=["age_years","diagnosis"])
    enc = OrdinalEncoder()
    clinical_data_enc=enc.fit_transform(dataset_clinical_cat_df.to_numpy())
    clinical_cat_data_df=pd.DataFrame(data=clinical_data_enc,
                                      columns=dataset_clinical_cat_df.columns,
                                      index=dataset_clinical_cat_df.index)
    demographic_data_df=pd.concat([clinical_cat_data_df,age_df.round(1)],
                               axis=1,
                               join='inner')
    patient_data_df=pd.concat([demographic_data_df,diagnosis_df],
                               axis=1,
                               join='inner')
    ...
    plt.figure(figsize=(8, 8), dpi=300, facecolor='w', edgecolor='k')
    sns.countplot(x='diagnosis',
                  hue='inflammationstatus',
                  data=X_y_df)
    sns.despine()
    plt.savefig(dir_fig_preproc+'inflammation_counts.png',
                format='png',
                dpi=300,
                bbox_inches='tight')
    plt.show()
    ...
    sns.countplot(x='diagnosis',
                  ...
                  data=X_y_df)
    sns.despine()
    plt.savefig(dir_fig_preproc+'biopsy_counts.png',
                format='png',
                dpi=300,
                bbox_inches='tight')
    plt.show()
    ...
    clinical_data_df=X_df.iloc[:,range(0,7)]
    microbiome_data_df=X_df.iloc[:,range(7,num_columns)]
    cat_feat_indices=list(np.arange(6))
    ...
    X_fpr_new=np.round(SelectFpr(score_func=f_classif).fit_transform(power_transform(X_red),
                                    label_binarize(y_df.to_numpy(),classes=['CD','UC'])),2)
    ...
    num_selector = list(range(7,num_columns))
    ...
    scale_pos_weight_value=round(pos_neg_dict['CD']/pos_neg_dict['UC'],2)
    ...
    estimators = [
                    (
                     'HGBC_clin',
                     make_pipeline(ColumnSelector(range(0,7)),
                    ...
                    (
                      'MLP',
                      make_pipeline(linear_preprocessor,
                                    MLPClassifier(hidden_layer_sizes=(int(0.5*np.size(X_fpr_new, 1)),),
                                                  activation="tanh",
                                                  solver="adam",
                                                  alpha=1,
    ...
    y_train=label_binarize(y_train_df.to_numpy(),classes=['CD','UC'])
    y_test=label_binarize(y_test_df.to_numpy(),classes=['CD','UC'])
    ...
    plt.figure(dpi=300, facecolor='w', edgecolor='k', figsize=(16,9))            
